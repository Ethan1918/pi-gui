...
Build the current checkout; use an isolated profile/workspace through the shared Electron harness. Run the skill's read-only doctor before driving. Use the visible skill smoke for user-surface proof. Existing background core specs are supplementary regression checks; OS pickers/clipboard need native proof. Do not share input ownership or build output across concurrent runs. Provider execution requires an explicitly configured live lane; the bundled settings proof needs no login.
...
Record feature ID, entry point, command, actual result, and evidence path. Capture action plus result and verify side effects independently. Do not mark skipped, fixture-only, or unvisited entries as passed. Expand the map when another user entry point is found. The bundled smoke exercises Settings section navigation, Skills navigation, opening New thread and entering an unsent draft, and settings persistence. Archive, worktrees, provider execution, and native dialogs remain separate recipes. Opening a surface does not verify every action on it.
