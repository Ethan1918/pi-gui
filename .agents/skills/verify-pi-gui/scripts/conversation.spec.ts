...
import { launchDesktop, type DesktopHarness } from '../../../../apps/desktop/tests/helpers/electron-app';
...
test('real conversation: stream, switch, tool, stop, archive, restart', async () => {
  ...
  if (process.env.PI_APP_REAL_AUTH !== '1' || !source || !provider || !model) {
    throw new Error('BLOCKED: conversation proof requires explicit real-auth source, provider and model; it never skips to a pass');
  }
  ...
  const checkpoint = async (name: string) => {
    ...
    completed.push(name);
    await writeFile(join(evidence, 'progress.json'), JSON.stringify({ provider, model, completed, assertionFailures: test.info().errors.map((error) => error.message) }, null, 2));
  };
  const launch = async () => {
    ...
    const identity = await harness.electronApp.evaluate(({ app, BrowserWindow }) => ({
      ...
      visible: BrowserWindow.getAllWindows()[0]?.isVisible(), focused: BrowserWindow.getAllWindows()[0]?.isFocused(),
      testMode: process.env.PI_APP_TEST_MODE ?? null, testHooks: '__PI_APP_TEST_HOOKS' in globalThis,
    }));
    await writeFile(join(evidence, `${phase}-doctor.json`), JSON.stringify(identity, null, 2));
    expect(identity).toMatchObject({ visible: true, focused: true, testMode: null, testHooks: false });
  ...
  const close = async () => {
    ...
    harness = undefined;
    try {
      if (traceStarted) await current.electronApp.context().tracing.stop({ path: join(evidence, `${phase}.zip`) });
    } finally {
      traceStarted = false;
  ...
  const start = async (prompt: string) => {
    ...
    await expect(page.getByTestId('send')).toHaveAttribute('aria-label', 'Stop run', { timeout: 30_000 });
    const id = await page.locator('.session-row--active').getAttribute('data-session-id');
    expect(id).toBeTruthy();
    return id!;
  };
  const idle = async (id: string) => {
    await expect(row(id)).not.toHaveAttribute('data-sidebar-indicator', 'running', { timeout: 120_000 });
  ...
  try {
    ...
    await test.step('Send Alpha and observe growing assistant text while running', async () => {
      alpha = await start('Do not use tools. Write 60 numbered lines about simple software testing, at least 8 words per line. Begin with ALPHA_BEGIN and finish with ALPHA_DONE.');
      const samples: Array<{ at: number; length: number; running: boolean }> = [];
      try {
        await expect.poll(async () => {
          if (await page.getByTestId('composer-error-banner').count()) throw new Error(await page.getByTestId('composer-error-banner').innerText());
          const text = (await assistant().allTextContents()).join('\n');
          const running = await row(alpha).getAttribute('data-sidebar-indicator') === 'running';
          if (text.length && samples.at(-1)?.length !== text.length) samples.push({ at: Date.now(), length: text.length, running });
          return samples.filter((sample) => sample.running).length;
        }, { timeout: 100_000, intervals: [100] }).toBeGreaterThanOrEqual(2);
    ...
    await test.step('Run Bravo tool and switch to Alpha while Bravo continues', async () => {
      ...
      await expect.soft(page.getByTestId('composer'), 'Alpha draft must survive thread switches').toHaveValue(drafts.alpha);
      ...
      expect((await readFile(join(workspace, 'verification-tool.txt'), 'utf8')).trim()).toBe('BRAVO_TOOL_OK');
    ...
    await test.step('Send a follow-up and stop it through the composer', async () => {
      await page.getByTestId('composer').fill('Do not use tools. Write 1000 numbered sentences about testing. Start with CANCEL_BEGIN.');
      ...
      await expect.soft(page.getByTestId('composer'), 'Alpha draft must survive thread switches').toHaveValue(drafts.alpha);
      await select(bravo);
      await expect.soft(page.getByTestId('composer'), 'Bravo draft must survive thread switches/archive').toHaveValue(drafts.bravo);
      await checkpoint('draft-isolation');
    });
    await test.step('Archive and restore Bravo through the sidebar', async () => {
      await row(bravo).hover();
      await row(bravo).getByLabel(/^Archive /).click();
      ...
      await row(bravo).hover();
      await row(bravo).getByLabel(/^Restore /).click();
    ...
    await test.step('Verify both conversations and drafts after restart', async () => {
      ...
      for (const [id, marker, draft] of [[alpha, 'ALPHA_DONE', drafts.alpha], [bravo, 'BRAVO_DONE', drafts.bravo]]) {
        await select(id);
        await expect(assistant().filter({ hasText: marker })).toHaveCount(1);
        await expect.soft(page.getByTestId('composer'), 'Draft must survive restart').toHaveValue(draft);
        await checkpoint(id === alpha ? 'restart-alpha' : 'restart-bravo');
      }
    });
  } catch (error) {
    if (harness && page!) await page.screenshot({ path: join(evidence, 'failure.png') }).catch(() => {});
    await writeFile(join(evidence, 'result.json'), JSON.stringify({ result: 'failed', provider, model, completed, error: String(error) }, null, 2));
    throw error;
  } finally {
    await close();
  }
  await writeFile(join(evidence, 'result.json'), JSON.stringify({ result: test.info().errors.length ? 'failed' : 'passed', provider, model, completed, runs, assertionFailures: test.info().errors.map((error) => error.message) }, null, 2));
});
